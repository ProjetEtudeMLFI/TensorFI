...
from TensorFI.fiConfig import FIConfig, Ops
from TensorFI.fiLog import FILog
...
def initFIConfig(fiParams):
    ...
    if fiConf.faultSeed:
        np.random.seed(fiConf.faultSeed)
...
def perturb(val):
    ...
    if logInjection:
        logging.debug("\tPerturbing " + str(val) + " of type: " + str(vType) +
                      " isScalar: " + str(isScalar))
...
def condPerturb(op, res):
    ...
    if logInjection:
        logging.debug("\tChecking if operation " + str(op) +
                      " is chosen for injection")
    ...
    if fiConf.isSelected(op):
        ...
        if logInjection:
            logging.debug("\tOperation " + str(op) +
                          " is chosen for injection")
        ...
        if (count > fiConf.skipCount):
            ...
            if (fiConf.injectMode == "dynamicInstance"):
                ...
                if op not in randInstanceMap:
                    # random instance of the selected op to be injected
                    randInstanceMap[op] = np.random.randint(low=1,
                                                            high=instance + 1)

                # first instance of the op
                if op not in visitedOp:
                    visitedOp[op] = 1
                ...
                if (visitedOp[op] == randInstanceMap[op]):
                    res = perturb(res)
                    faultLog.updateInjectedInstance(randInstanceMap[op],
                                                    instance)
                    faultLog.commit()
            ...
            if (fiConf.injectMode == "oneFaultPerRun"):
                ...
                if (injectedOp == 0):
                    injectedOp = np.random.randint(low=1,
                                                   high=totalInstance + 1)
...
def createInjectFaultCast(type):
    ...
    castInto = castType(
        type)  # get the appropriate casting function for the type
    ...
    def injectFaultCast(a, b=None):
        ...
        if logReturn:
            logging.debug("\tReturning " + str(res))
        return res
...
def injectFaultAssign(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Assignment " + str(res))
    return res
...
def injectFaultIdentity(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Identity " + str(res))
    return res
...
def injectFaultAdd(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Add " + str(res))
    return res
...
def injectFaultSub(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Sub " + str(res))
    return res
...
def injectFaultMul(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Mul " + str(res))
    return res
...
def injectFaultSquare(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Square " + str(res))
    return res
...
def injectFaultShape(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Shape " + str(res))
    return res
...
def injectFaultSize(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Size " + str(res))
    return res
...
def injectFaultFill(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Fill" + str(res))
    return res
...
def injectFaultFloorMod(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from FloorMod " + str(res))
    return res
...
def injectFaultRange(start, stop, step, dtype=None):
    ...
    if logReturn:
        logging.debug("\tReturning from Range " + str(res))
    return res
...
def injectFaultRank(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Rank " + str(res))
    return res
...
def injectFaultSum(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Sum " + str(res))
    return res
...
def injectFaultReshape(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Reshape " + str(res))
    return res
...
def injectFaultOneHot(a, b, c, d):
    ...
    if logReturn:
        logging.debug("\tReturning from One Hot " + str(res))
    return res
...
def injectFaultMatMul(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from MatMul " + str(res))
    return res
...
def injectFaultArgMax(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from ArgMax " + str(res))
    return res
...
def injectFaultArgMin(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from ArgMin " + str(res))
    return res
...
def injectFaultEqual(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Equal " + str(res))
    return res
...
def injectFaultNotEqual(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Not Equal " + str(res))
    return res
...
def injectFaultLessEqual(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Less Equal " + str(res))
    return res
...
def injectFaultGreaterEqual(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Greater Equal " + str(res))
    return res
...
def injectFaultMean(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Mean " + str(res))
    return res
...
def injectFaultCountNonZero(a):
    ...
    if logReturn:
        logging.debug("\tReturning on CountNonZero " + str(res))
    return res
...
def injectFaultConv2D(a, b, strides, padding):
    ...
    if logReturn:
        logging.debug("\tReturning from Conv2D " + str(res))
    return res
...
def injectFaultMaxPool(a, ksize, strides, padding):
    ...
    if logReturn:
        logging.debug("\tReturningfrom MaxPool  " + str(res))
    return res
...
def injectFaultUnpack(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Unpack " + str(res))
    return res
...
def injectFaultStridedSlice(a, b, c, d):
    ...
    if logReturn:
        logging.debug("\tReturning from StridedSlice " + str(res))
    return res
...
def injectFaultExpandDims(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from ExpandDims " + str(res))
    return res
...
def injectFaultConcatV2(a, b, c):
    ...
    res = condPerturb(Ops.PACK, res)
    if logReturn:
        logging.debug("\tReturning from Concat " + str(res))
    return res
...
def injectFaultSoftmax(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Softmax " + str(res))
    return res
...
def injectFaultMaximum(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Maximum " + str(res))
    return res
...
def injectFaultMinimum(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Minimum " + str(res))
    return res
...
def injectFaultSwitch(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Switch " + str(res))
    return res
...
def injectFaultGreater(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Greater " + str(res))
    return res
...
def injectFaultNeg(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Neg " + str(res))
    return res
...
def injectFaultPow(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from Pow " + str(res))
    return res
...
def injectFaultAbs(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Abs " + str(res))
    return res
...
def injectFaultRsqrt(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Rsqrt " + str(res))
    return res
...
def injectFaultNN(a, b, c):
    ...
    if logReturn:
        logging.debug("\tReturning from Nearest Neighbors " + str(res))
    return res
...
def injectFaultLog(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Log " + str(res))
    return res
...
def injectFaultRealDiv(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from RealDiv " + str(res))
    return res
...
def injectFaultBiasAdd(a, b):
    ...
    if logReturn:
        logging.debug("\tReturning from BiasAdd " + str(res))
    return res
...
def injectFaultSigmoid(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Sigmoid " + str(res))
    return res
...
def injectFaultTanh(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Tanh " + str(res))
    return res
...
def injectFaultLRN(a, bias, alpha, beta):
    ...
    if logReturn:
        logging.debug("\tReturning from LRN " + str(res))
    return res
...
def injectFaultELU(a):
    ...
    if logReturn:
        logging.debug("\tReturning from ELU " + str(res))
    return res
...
def injectFaultRandomUniform(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Random Uniform " + str(res))
    return res
...
def injectFaultFloor(a):
    ...
    if logReturn:
        logging.debug("\tReturning from Floor " + str(res))
    return res
...
# Otherwise, they will all raise NotImplementedError(OpName)
...
def injectFaultGeneric(*inputs):
    ...
    if logReturn:
        logging.debug("\tReturning " + str(outputs))
    return outputs
...
opTable = {
    ...
    "Max":
    injectFaultMaximum,  # FIXME: Not sure if Max is a synonymn of Maximum or a new operation
    "Minimum": injectFaultMinimum,
    "Min":
    injectFaultMinimum,  # FIXME: Not sure if Min is a synonymn of Minimum or a new operation
