...
from TensorFI.fiConfig import configFaultParams
from TensorFI.injectFault import initFIConfig, initFILog, logRun
        ...
        def newRun(tensorList=None, feed_dict=None, useCached=False):
            ...
            if maintainCache:  # If we wish to maintain a cache at runtime
                if useCached:
                    ...
                    cache[
                        "list"] = tensorList  # Keep track of prior invocation list
                    cache[
                        "dict"] = feed_dict  # Keep track of the prior feed_dict

            # TensorList must not be None at this point as it'd have been loaded from cache
            if tensorList is None:
                raise ValueError("tensorList cannot be None")
            ...
            if self.noInjections:
                logging.info("No injections: Calling oldRun on " +
                             str(tensorList))
                return oldRun(tensorList, feed_dict)

            # Otherwise, start the fault injector
            self.runCount = self.runCount + 1
            logging.info("Calling newRun " + self.name + " runCount = " +
                         str(self.runCount))
...
# Functions to get and set the logging level - cn be called externally
    ...
    def __init__(
        ...
        fiPrefix="fi_"
    ):  # Prefix to attach to each node inserted for fault injection
    ...
    def launch(self,
               numInjections,
               computeDiff,
               collectStats=fiStats.defaultStats,
               myID=0):
        ...
        )  # Enable injections if they've been disabled previously

        logging.info("Starting " + str(numInjections) +
                     " injections: Process " + str(myID))
        ...
        for i in range(numInjections):
            ...
            # Done with this run
            logging.info("Done experiment " + str(i) + " Process: " +
                         str(myID))
        # End for loop
    ...
    def pLaunch(self,
        ...
        numInjectionsPerProcess = int(
            floor(numberOfInjections / numberOfProcesses))
        ...
        processes = [
        ]  # List of processes or Threads depending on the value of useProcesses
        ...
        for i in range(numberOfProcesses):
            ...
            if (i == numberOfProcesses - 1):
                numInjectionsPerProcess += extraInjections
            ...
            if not parallel:
                ...
                argArray.append(i)
                logging.info("Launching process " + str(i) +
                             " with arguments " + str(argArray))
                if useProcesses:
