...
def randomElementTensor(dtype, val):
    ...
    if (dim == 1):
        ...
        index = [
            np.random.randint(low=0, high=(val.shape[0])),
            np.random.randint(low=0, high=(val.shape[1]))
        ]
...
def float2bin(number, decLength=10):
    ...
    for x in range(decLength):
        ...
        integer, decimal = format((decimalConverter(decimal)) * 2,
                                  '.10f').split(".")
        res += integer
