...
from TensorFI import faultTypes
...
class FIConfig(object):
    ...
    faultTypeMap = {
        FaultTypes.NONE.value: (faultTypes.noScalar, faultTypes.noTensor),
        FaultTypes.RAND.value:
        (faultTypes.randomScalar, faultTypes.randomTensor),
        FaultTypes.ZERO.value: (faultTypes.zeroScalar, faultTypes.zeroTensor),
        FaultTypes.ELEM.value:
        (faultTypes.randomElementScalar, faultTypes.randomElementTensor),
        FaultTypes.ELEMbit.value:
        (faultTypes.bitElementScalar, faultTypes.bitElementTensor),
        FaultTypes.RANDbit.value: (faultTypes.bitScalar, faultTypes.bitTensor)
    }
    ...
    def faultConfigType(self, faultTypeScalar, faultTypeTensor):
        ...
        if faultTypeScalar in self.faultTypeMap:
        ...
        if faultTypeTensor in self.faultTypeMap:
    ...
    def isSelected(self, op):
        ...
        return op in self.injectMap or Ops.ALL in self.injectMap
    ...
    def __init__(self, fiParams):
        ...
        if Fields.Ops.value in fiParams:
            opsList = fiParams[Fields.Ops.value]
            if opsList is not None:
        ...
        if Fields.Instances.value in fiParams:
        ...
        if Fields.Seed.value in fiParams:
        ...
        # default value is 0
        if Fields.SkipCount.value in fiParams:
...
def staticFaultParams():
    ...
    params[Fields.ScalarFaultType.
           value] = FaultTypes.RAND.value  # Scalar Fault type
    params[Fields.TensorFaultType.
           value] = FaultTypes.RAND.value  # Tensor Fault type
