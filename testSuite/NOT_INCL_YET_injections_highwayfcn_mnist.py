...
def highway_fc_layer(input,
    ...
    H = activation(tf.matmul(input, weights_normal) + bias_normal,
                   name="Input_gate")
    #Step 4. calculate transform game
    T = tf.nn.sigmoid(tf.matmul(input, weights_transform) + bias_transform,
                      name="T_gate")
...
for layer in range(number_of_layers):
    ...
    if layer == 0:
        ...
        output = fully_connected_layer(prev_layer,
                                       hidden_size,
                                       output_size,
                                       activation=None)
    else:
        # for any layer between input and output layer
        prev_layer = highway_fc_layer(prev_layer,
                                      hidden_size,
                                      carry_b=cary_bias)
...
cost = tf.reduce_sum(
    tf.nn.softmax_cross_entropy_with_logits(
        logits=output, labels=targets))  #this is standard cross entropy loss
...
def optimize():
    ...
    for i in range(epochs):
        ...
        for ii in range(mnist_data.train.num_examples // batch_size):
            ...
            c, _ = session.run([cost, optimizer],
                               feed_dict={
                                   inputs: imgs,
                                   targets: labs
                               })
        ...
        print("Epoch: {}/{}".format(i + 1, epochs),
              " | Current loss: {}".format(np.mean(epoch_cost)),
...
def test_model():
    ...
    return session.run(accuracy,
                       feed_dict={
                           inputs: mnist_data.test.images[:256],
