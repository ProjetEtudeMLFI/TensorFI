...
def fake_data(num_images):
    ...
    data = numpy.ndarray(shape=(num_images, IMAGE_SIZE, IMAGE_SIZE,
                                NUM_CHANNELS),
                         dtype=numpy.float32)
...
def error_rate(predictions, labels, isPrint=False):
    ...
    return (100.0 -
            (100.0 * numpy.sum(numpy.argmax(predictions, 1) == labels) /
...
def main(_):
    ...
    def model(data, train=False):
        ...
        a = [1, 1, 1, 1]
        conv = tf.nn.conv2d(data, conv1_weights, a, padding='SAME')
    ...
    with tf.compat.v1.Session() as sess:
        ...
        # inject ten inputs
        for i in range(10):
            each = indexOfCorrectSample[
                i]  # index of the sample to be injected
            newData = (test_data[each].reshape(1, 28, 28,
                                               1))  # resize the data
            ...
            for j in range(fiCount):
                test_error, _ = error_rate(eval_in_batches(newData, sess),
                                           newLab, True)
                # FI results in SDC
                if (test_error == 100.):
                    sdcCount += 1
