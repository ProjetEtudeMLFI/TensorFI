...
num_input = 3  # 3 features in survive datset
num_classes = 2  # whether a patient survives or not ==> 2 classes
...
with tf.compat.v1.Session() as sess:
    ...
    orgAcy = accuracy.eval({X: batch_xs[:150], Y: batch_ys[:150]})
    fi.turnOnInjections()
    ...
    fiAcy = accuracy.eval({X: batch_xs[:150], Y: batch_ys[:150]})
    ###
