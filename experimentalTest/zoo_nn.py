...
num_input = 16  # 16 features in zoo dataset
num_classes = 7  # 7 classes of animals
...
with tf.compat.v1.Session() as sess:
    ...
    orgAcy = accuracy.eval({X: batch_xs[:50], Y: batch_ys[:50]})
    fi.turnOnInjections()
    ...
    fiAcy = accuracy.eval({X: batch_xs[:50], Y: batch_ys[:50]})
